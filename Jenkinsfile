...
def check_post_curl()
{
result = curl -d "{"firstname" :"ben", "lastname": "goldenberg", "id": 2, "class": "D2"}" -H "Content-Type: application/json" -s -w %{http_code}} "${env.path}/school/students"
script{
if (result.contain(200)){
    ok = "Ok"}
else{
...
def check_put_curl()
{
result = curl -s -w %{http_code}} -X PUT "${env.path}/school/students/2/6"
script{
if (result.contain(200)){
    ok = "Ok"}
else{
